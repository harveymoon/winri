...
pub fn load() -> Result<Root, LoadError> {
    ...
    from_koto_value(config_map.clone()).map_err(|e| LoadError::BadScriptExport(e.to_string()))
}

struct KDeserializer(KValue);

struct DesKFunction(KFunction);

impl serde::de::Deserializer<'_> for KDeserializer {
    type Error = koto::serde::Error;
